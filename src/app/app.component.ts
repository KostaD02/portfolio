...
export class AppComponent implements OnInit, OnDestroy {
  ...
  public isAnimationOn: boolean = sessionStorage.getItem('is_animation_off')
    ? false
    : true;

  public hideEventEmitter: EventEmitter<boolean> = new EventEmitter();
  public startedScrollingEventEmitter: EventEmitter<boolean> =
    new EventEmitter();
  ...
  ngOnInit(): void {
    sessionStorage.setItem('is_animation_off', 'true');
    AOS.init({
      once: true,
    });
    setTimeout(() => {
      // ? using for close animation loading
